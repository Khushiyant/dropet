...
```shell
  docker buildx build -t khushiyant/dropet:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install khushiyant/dropet:latest
```
...
```shell
  docker extension dev ui-source khushiyant/dropet:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug khushiyant/dropet:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset khushiyant/dropet:latest
```
...
```shell
docker extension update khushiyant/dropet:latest
```
...
```shell
docker extension rm khushiyant/dropet:latest
```
